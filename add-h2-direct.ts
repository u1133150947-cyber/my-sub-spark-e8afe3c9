...
async function run() {
  ...
  const commonSettings = { clients: [], obfs: "", obfsPassword: "" };
  const commonSniffing = { enabled: true, destOverride: ['http', 'tls', 'quic'], routeOnly: false };
  ...
  try {
    await addInbound(czUrl, czUser, czPass, {
      ...
      settings: JSON.stringify(commonSettings),
      streamSettings: JSON.stringify({ network: 'hysteria2', security: 'tls', tlsSettings: { serverName: 'cz.panelsu.ru', certificates: [{ certificateFile: '/root/cert/cz.panelsu.ru/fullchain.pem', keyFile: '/root/cert/cz.panelsu.ru/privkey.pem' }] } }),
      sniffing: JSON.stringify(commonSniffing)
    });
    console.log('CZ done');
  } catch (e) { console.error('CZ Error:', e); }
  ...
  try {
    await addInbound(ruUrl, ruUser, ruPass, {
      ...
      listen: '', port: 44433, protocol: 'hysteria2',
      settings: JSON.stringify(commonSettings),
      streamSettings: JSON.stringify({ network: 'hysteria2', security: 'tls', tlsSettings: { serverName: 'ru.panelsu.ru', certificates: [{ certificateFile: '/var/lib/caddy/.local/share/caddy/certificates/acme-v02.api.letsencrypt.org-directory/ru.panelsu.ru/ru.panelsu.ru.crt', keyFile: '/var/lib/caddy/.local/share/caddy/certificates/acme-v02.api.letsencrypt.org-directory/ru.panelsu.ru/ru.panelsu.ru.key' }] } }),
      sniffing: JSON.stringify(commonSniffing)
    });
    console.log('RU done');
  } catch (e) { console.error('RU Error:', e); }
}
run();
