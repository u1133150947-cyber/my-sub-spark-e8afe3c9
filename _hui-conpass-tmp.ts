...
sqlite3 $DB "UPDATE account SET con_pass='${VERN_UUID}' WHERE username='vern';"
sqlite3 $DB "SELECT id,username,con_pass FROM account WHERE username='vern';"
echo '--- auth with bare uuid ---'
curl -ksS --http1.1 -A 'Mozilla/5.0' -X POST https://127.0.0.1:8081/hui/hysteria2/auth -H 'Content-Type: application/json' -d '{"auth":"${VERN_UUID}","addr":"1.2.3.4:1234","tx":0}'; echo
`));
