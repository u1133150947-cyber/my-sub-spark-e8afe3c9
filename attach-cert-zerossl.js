...
conn.on('ready', () => {
  ...
  const cmd = [
    ...
    `/root/.acme.sh/acme.sh --issue -d ${domain} --standalone --httpport 80 --force --server zerossl`,
    `mkdir -p /root/cert/${domain}`,
    `/root/.acme.sh/acme.sh --installcert -d ${domain} --key-file /root/cert/${domain}/privkey.pem --fullchain-file /root/cert/${domain}/fullchain.pem --server zerossl`,
