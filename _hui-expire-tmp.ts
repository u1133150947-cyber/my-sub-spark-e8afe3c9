...
// expire_time looks like ms. Set to 10 years from now
const EXP = Date.now() + 10*365*24*3600*1000;
console.log(await ssh(`set +e
DB=/usr/local/h-ui/data/h_ui.db
sqlite3 $DB "UPDATE account SET expire_time=${EXP}, quota=1099511627776 WHERE username='vern';"  -- 1TB quota, exp +10y
sqlite3 $DB "SELECT id,username,expire_time,quota,download,upload,kick_util_time,deleted FROM account WHERE username='vern';"
echo '--- auth vern ---'
...
echo '--- recent h-ui auth log ---'
journalctl -u h-ui -n 50 --no-pager | grep -E 'hysteria2/auth|ERROR|error|account|expir' | tail -20
`));
