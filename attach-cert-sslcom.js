...
conn.on('ready', () => {
  ...
  const cmd = [
    ...
    `/root/.acme.sh/acme.sh --register-account -m admin@${domain} --server ssl.com`,
    `/root/.acme.sh/acme.sh --set-default-ca --server ssl.com`,
    `/root/.acme.sh/acme.sh --issue -d ${domain} --standalone --httpport 80 --force --server ssl.com`,
    `mkdir -p /root/cert/${domain}`,
    `/root/.acme.sh/acme.sh --installcert -d ${domain} --key-file /root/cert/${domain}/privkey.pem --fullchain-file /root/cert/${domain}/fullchain.pem --server ssl.com`,
