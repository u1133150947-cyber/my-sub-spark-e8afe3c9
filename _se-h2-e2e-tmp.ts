...
ls -la /tmp/hyse.json
rm -f /tmp/hyse.log /tmp/hyse.pid
( "$XRAY" run -c /tmp/hyse.json > /tmp/hyse.log 2>&1 & echo $! > /tmp/hyse.pid )
for i in $(seq 1 20); do ss -lnt | grep -q ':19091' && break; sleep 0.2; done
ss -lnt | grep ':19091' || echo NO_LISTEN
...
kill $(cat /tmp/hyse.pid) 2>/dev/null
true
`;
