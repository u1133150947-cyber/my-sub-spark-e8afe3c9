...
const cmd = `cd /opt/sub-manager && cat << 'TS' > test-deployed-addClient.ts
...
async function main() {
  try {
    ...
    const res = await addClient("pee9e3676f7", 3, c, "hysteria2");
...
main();
TS
deno run -A test-deployed-addClient.ts
`;
