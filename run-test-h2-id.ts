...
const cmd = `cd /opt/sub-manager && cat << 'TS' > test-h2-id.ts
...
async function main() {
  ...
  const settings = JSON.stringify({
    clients: [{ id: "testpass123", email: "client_test4", limitIp: 0, totalGB: 0, expiryTime: 0, enable: true }]
  });
