...
conn.on('ready', () => {
  ...
  const cmd = [
    ...
    `/root/.acme.sh/acme.sh --issue -d ${domain} -d www.${domain} --standalone --httpport 80 --force --server letsencrypt`,
    `mkdir -p /root/cert/${domain}`,
    `/root/.acme.sh/acme.sh --installcert -d ${domain} -d www.${domain} --key-file /root/cert/${domain}/privkey.pem --fullchain-file /root/cert/${domain}/fullchain.pem`,
