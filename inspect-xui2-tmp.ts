...
const c = new Client();
c.on('ready', () => c.exec(`sqlite3 /etc/x-ui/x-ui.db "SELECT key FROM settings WHERE key LIKE '%xray%' OR key LIKE '%template%';" ; echo ===; ls /usr/local/x-ui/bin/ 2>/dev/null; ls /etc/x-ui/`, (e, s) => {
